function varargout = zpgui3(varargin)
...
set(0,'defaultaxesfontsize',22);
    ...
    uicontrol('style','pushbutton',...
         ...
         'units','normalized',...
         'position',[.55 .6 .2 .1],...
         'callback','zpgui3(''addzero'')');
    uicontrol('style','pushbutton',...
         'string','Remove Zero',...
         'fontsize',14,...
         'units','normalized',...
         'position',[.8 .6 .2 .1],...
         'callback','zpgui3(''removezero'')');
    uicontrol('style','pushbutton',...
         'units','normalized',...
         'fontsize',14,...
         'position',[.55 .8 .2 .1],'string','Add Pole',...
         'callback','zpgui3(''addpole'')');
    uicontrol('style','pushbutton',...
         'units','normalized',...
         'fontsize',14,...
         'position',[.8 .8 .2 .1],'string','Remove Pole',...
         'callback','zpgui3(''removepole'')');
    ...
    Y = fft(b,Nfft)./fft(a,Nfft);  Y=Y/max(abs(Y));
    Lresp = plot(2*pi*((0:Nfft-1)/Nfft - .5), (fftshift(abs(Y))));
    ax2 = gca;
    set(ax2,'xlim',[-pi pi])
    set(ax2,'ylim',[0 1])
    grid on
    ...
    set(Lresp,'erasemode','xor','linewidth',4)

    set(zh,'buttondownfcn','zpgui3(''zeroclick'')',...
        'markersize',14,'linewidth',4)
    set(ph,'buttondownfcn','zpgui3(''poleclick'')',...
        'markersize',14,'linewidth',4)
    
case 'addzero'
    if length(zh)>0
        zh(end+1) =  line(.5,0,'parent',ax1,'buttondownfcn','zpgui3(''zeroclick'')',...
        'markersize',14,'linewidth',4,'marker','o','linestyle','none');
    else
        zh = line(.5,0,'parent',ax1,'buttondownfcn','zpgui3(''zeroclick'')',...
        'markersize',14,'linewidth',4,'marker','o','linestyle','none');
...
case 'addpole'
    if length(ph)>0
        ph(end+1) =  line(.5,0,'parent',ax1,'buttondownfcn','zpgui3(''poleclick'')',...
        'markersize',14,'linewidth',4,'marker','x');
    else
        ph =  line(.5,0,'parent',ax1,'buttondownfcn','zpgui3(''poleclick'')',...
        'markersize',14,'linewidth',4,'marker','x');
...
case 'recompute'
    ...
    Y = fft(b,Nfft)./fft(a,Nfft); Y=Y/max(abs(Y));
    set(Lresp,'ydata',(fftshift(abs(Y))))
